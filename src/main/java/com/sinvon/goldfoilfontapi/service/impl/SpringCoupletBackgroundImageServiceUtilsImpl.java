...
@Service
public class SpringCoupletBackgroundImageServiceUtilsImpl implements SpringCoupletBackgroundImageUtilsService {
    ...
    public void createSpringCoupletBackgroundImage(int width, int height) {
    ...
    // 随机绘制图案
    private void drawRandomElements(Graphics2D g2d) {
        // 随机选择吉祥符号
        if (rand.nextBoolean()) {
            drawLuckySymbol(g2d);
        }
    ...
    // 绘制节日特色（例如烟花）
    private void drawFestivalElements(Graphics2D g2d) {
    ...
    // 绘制吉祥的动物（例如龙、凤、蝙蝠等）
    private void drawLuckyAnimals(Graphics2D g2d) {
        // 绘制一只简化版的龙（象征龙的形状）
